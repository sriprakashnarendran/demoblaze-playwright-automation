...
export class ApiHelper {
  ...
  async login(username: string, password: string): Promise<string> {
    ...
    const body = String(
      await this.validate(apiConfig.allure.login, response)
    );

    const token = body
      .match(new RegExp(`${apiConfig.response.authToken}:\\s*([^"]+)`))?.[1]
      ?.trim();
  ...
  async validateCart(token: string, productId: number): Promise<void> {
    ...
    expect(
      items.length,
      `Cart is empty: ${JSON.stringify(body)}`
    ).toBeGreaterThan(0);

    expect(
      items.some(
        (item: any) =>
          Number(this.productId(item)) === Number(productId)
      )
    ).toBeTruthy();
  }
  ...
  async waitForCartProduct(
    ...
    while (Date.now() - start < timeout) {
      ...
      await new Promise(resolve =>
        setTimeout(resolve, apiConfig.defaults.pollingInterval)
      );
    }

    throw new Error(
      `Product ${productId} was not available in cart within ${timeout}ms`
    );
  }
