...
test("@regression UI E2E - add product to cart and checkout", async ({ homePage, productPage, cartPage }) => {
  ...
  await allure.step("Enter checkout details and purchase", async () => {
    await cartPage.checkout(data.checkout.name, data.checkout.country, data.checkout.city, data.checkout.card, data.checkout.month, data.checkout.year);
  });
